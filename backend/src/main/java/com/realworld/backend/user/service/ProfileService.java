...
public class ProfileService {
  ...
  public ResponseProfile getProfile(String username) {
    User searchUser = userRepository.findById(username).orElseThrow(() ->new CustomException(CustomExceptionList.USER_NOT_FOUND));
    ...
    return new ResponseProfile(new ResponseProfileDetails(searchUser.getUsername(),
        searchUser.getBio(), searchUser.getImage(), isfollowing));
  }
  ...
  public ResponseProfile followUser(String username) {
    ...
    User searchUser = userRepository.findById(username).orElseThrow(() ->new CustomException(CustomExceptionList.USER_NOT_FOUND));
    Follow follow = followRepository.findByFromUserNameAndToUserName(currentUsername, username).orElse(null);
    if (follow == null) {
      followRepository.save(new Follow(currentUsername, username));
    }
    return new ResponseProfile(new ResponseProfileDetails(searchUser.getUsername(),
        searchUser.getBio(), searchUser.getImage(), true));
  }
  ...
  public ResponseProfile unfollowUser(String username) {
    ...
    return new ResponseProfile(new ResponseProfileDetails(searchUser.getUsername(),
        searchUser.getBio(), searchUser.getImage(), false));
  }
}
